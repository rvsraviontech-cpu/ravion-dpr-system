...
<div class="mx-auto max-w-full">
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 sm:text-3xl">Add Material Receipt</h1>
            <p class="mt-1 text-gray-500">Enter what physically arrived at site. No activity or work-package classification is required.</p>
        </div>
    ...
    <form method="POST" action="{{ route('material-received.store') }}" id="material-receipt-form" enctype="multipart/form-data">
        @csrf
        ...
        <div class="sticky bottom-[68px] z-30 mt-6 grid grid-cols-1 gap-3 border-t border-gray-200 bg-white/95 py-3 backdrop-blur sm:flex sm:flex-wrap lg:static lg:border-0 lg:bg-transparent lg:py-0">
            <button type="submit" class="w-full rounded-xl bg-blue-600 px-7 py-3.5 font-semibold text-white shadow-sm hover:bg-blue-700 sm:w-auto sm:py-3">Save Material Receipt</button>
            <a href="{{ route('material-received.index') }}" class="w-full rounded-xl bg-gray-500 px-7 py-3.5 text-center font-semibold text-white hover:bg-gray-600 sm:w-auto sm:py-3">Cancel</a>
        </div>
    </form>
</div>
