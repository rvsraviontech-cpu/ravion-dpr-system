...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">
                Add a controlled designation and map it to a department.
            </p>
        </div>
    ...
    <form method="POST"
          action="{{ route('employee-designations.store') }}"
          class="space-y-5">
                    ...
                    <select name="department_id"
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}"
                                {{ (string) old('department_id', request('department_id')) === (string) $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
