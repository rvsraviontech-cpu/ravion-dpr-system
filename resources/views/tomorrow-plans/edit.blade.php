...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-800">Edit Tomorrow Plan</h1>
        <p class="text-gray-500 mt-1">Update draft tomorrow plan details.</p>
    </div>
...
<form method="POST" action="{{ route('tomorrow-plans.update', $tomorrowPlan) }}">
            ...
            <select name="project_block_id" class="border p-2 rounded w-full">
                ...
                @foreach($projectBlocks as $block)
                    <option value="{{ $block->id }}"
                        {{ old('project_block_id', $tomorrowPlan->project_block_id) == $block->id ? 'selected' : '' }}>
                        {{ $block->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="project_floor_id" class="border p-2 rounded w-full">
                ...
                @foreach($projectFloors as $floor)
                    <option value="{{ $floor->id }}"
                        {{ old('project_floor_id', $tomorrowPlan->project_floor_id) == $floor->id ? 'selected' : '' }}>
                        {{ $floor->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="project_unit_id" class="border p-2 rounded w-full">
                ...
                @foreach($projectUnits as $unit)
                    <option value="{{ $unit->id }}"
                        {{ old('project_unit_id', $tomorrowPlan->project_unit_id) == $unit->id ? 'selected' : '' }}>
                        {{ $unit->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="project_room_id" class="border p-2 rounded w-full">
                ...
                @foreach($projectRooms as $room)
                    <option value="{{ $room->id }}"
                        {{ old('project_room_id', $tomorrowPlan->project_room_id) == $room->id ? 'selected' : '' }}>
                        {{ $room->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="project_subspace_id" class="border p-2 rounded w-full">
                ...
                @foreach($projectSubspaces as $subspace)
                    <option value="{{ $subspace->id }}"
                        {{ old('project_subspace_id', $tomorrowPlan->project_subspace_id) == $subspace->id ? 'selected' : '' }}>
                        {{ $subspace->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="activity_id" id="activity_id" class="border p-2 rounded w-full" required>
                ...
                @foreach($activities as $activity)
                    <option value="{{ $activity->id }}"
                            ...
                            {{ old('activity_id', $tomorrowPlan->activity_id) == $activity->id ? 'selected' : '' }}>
            ...
            <select name="contractor_id" class="border p-2 rounded w-full">
                ...
                @foreach($contractors as $contractor)
                    <option value="{{ $contractor->id }}"
                        {{ old('contractor_id', $tomorrowPlan->contractor_id) == $contractor->id ? 'selected' : '' }}>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        ...
        <div>
            ...
            <input type="number" name="planned_labour"
        ...
        <div>
            ...
            <input type="number" name="required_skilled_labour"
        ...
        <div>
            ...
            <input type="number" name="required_semiskilled_labour"
        ...
        <div>
            ...
            <input type="number" name="required_helpers"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
            ...
            <textarea name="materials_required" rows="3"
                      class="border p-2 rounded w-full">{{ old('materials_required', $tomorrowPlan->materials_required) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-semibold mb-1">Machinery Required</label>
            <textarea name="machinery_required" rows="3"
                      class="border p-2 rounded w-full">{{ old('machinery_required', $tomorrowPlan->machinery_required) }}</textarea>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            ...
            <textarea name="risks_constraints" rows="4"
                      class="border p-2 rounded w-full">{{ old('risks_constraints', $tomorrowPlan->risks_constraints) }}</textarea>
        </div>

        <div>
            <label class="block text-sm font-semibold mb-1">Remarks</label>
            <textarea name="remarks" rows="4"
                      class="border p-2 rounded w-full">{{ old('remarks', $tomorrowPlan->remarks) }}</textarea>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectedActivityId = "{{ old('activity_id', $tomorrowPlan->activity_id) }}";
