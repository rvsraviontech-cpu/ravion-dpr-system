...
<div class="mx-auto max-w-full">
    ...
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-gray-500">
                Update draft tomorrow plan details.
            </p>
        </div>

        <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
            <a href="{{ route('tomorrow-plans.show', $tomorrowPlan) }}"
               class="inline-flex items-center justify-center rounded-lg bg-slate-700 px-5 py-3 text-center font-semibold text-white hover:bg-slate-800 sm:py-2.5">
                View
            </a>

            <a href="{{ route('tomorrow-plans.index') }}"
               class="inline-flex items-center justify-center rounded-lg bg-gray-600 px-5 py-3 text-center font-semibold text-white hover:bg-gray-700 sm:py-2.5">
                Back
            </a>
        </div>
    </div>
    ...
    <form method="POST"
          action="{{ route('tomorrow-plans.update', $tomorrowPlan) }}"
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Update project, date and priority.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    <select name="project_id"
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}"
                                {{ (string) old('project_id', $tomorrowPlan->project_id) === (string) $project->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="date"
                           name="planned_date"
                           value="{{ old('planned_date', $tomorrowPlan->planned_date) }}"
                           class="{{ $inputClass }}"
                           required>
                </div>
                ...
                <div>
                    ...
                    <select name="priority"
                            class="{{ $inputClass }}"
                            required>
                        <option value="Normal" {{ old('priority', $tomorrowPlan->priority) === 'Normal' ? 'selected' : '' }}>Normal</option>
                        <option value="Urgent" {{ old('priority', $tomorrowPlan->priority) === 'Urgent' ? 'selected' : '' }}>Urgent</option>
                        <option value="Critical" {{ old('priority', $tomorrowPlan->priority) === 'Critical' ? 'selected' : '' }}>Critical</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Location options are filtered from the selected project.</p>
            </div>
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Activity is filtered by Activity Division.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-5">
                <div>
                    <label class="{{ $labelClass }}">Activity Division</label>
                    <select id="activity_division_id" class="{{ $inputClass }}">
                ...
                <div>
                    ...
                    <select name="activity_id" id="activity_id" class="{{ $inputClass }}" required>
                ...
                <div>
                    ...
                    <input type="number"
                           ...
                           value="{{ old('planned_quantity', $tomorrowPlan->planned_quantity) }}"
                           class="{{ $inputClass }}"
                           required>
                </div>
                ...
                <div>
                    ...
                    <input type="text"
                           name="unit"
                           id="unit"
                           value="{{ old('unit', $tomorrowPlan->unit) }}"
                           class="{{ $inputClass }}"
                    ...
                    <select name="contractor_id" class="{{ $inputClass }}">
                        ...
                        @foreach($contractors as $contractor)
                            <option value="{{ $contractor->id }}"
                                {{ (string) old('contractor_id', $tomorrowPlan->contractor_id) === (string) $contractor->id ? 'selected' : '' }}>
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Update manpower required for tomorrow's work.</p>
            </div>

            <div class="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
                <div>
                    <label class="{{ $labelClass }}">Total Planned Labour</label>
                    <input type="number" min="0" name="planned_labour"
                           value="{{ old('planned_labour', $tomorrowPlan->planned_labour) }}"
                           class="{{ $inputClass }}">
                </div>

                <div>
                    <label class="{{ $labelClass }}">Skilled Labour</label>
                    <input type="number" min="0" name="required_skilled_labour"
                           value="{{ old('required_skilled_labour', $tomorrowPlan->required_skilled_labour) }}"
                           class="{{ $inputClass }}">
                </div>

                <div>
                    <label class="{{ $labelClass }}">Semi-Skilled</label>
                    <input type="number" min="0" name="required_semiskilled_labour"
                           value="{{ old('required_semiskilled_labour', $tomorrowPlan->required_semiskilled_labour) }}"
                           class="{{ $inputClass }}">
                </div>

                <div>
                    <label class="{{ $labelClass }}">Helpers</label>
                    <input type="number" min="0" name="required_helpers"
                           value="{{ old('required_helpers', $tomorrowPlan->required_helpers) }}"
                           class="{{ $inputClass }}">
                </div>
            </div>
        </div>
        ...
        <div class="mb-6 rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Update resources and approvals needed before work starts.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="{{ $labelClass }}">Materials Required</label>
                    <textarea name="materials_required" rows="3" class="{{ $inputClass }}">{{ old('materials_required', $tomorrowPlan->materials_required) }}</textarea>
                </div>

                <div>
                    <label class="{{ $labelClass }}">Machinery Required</label>
                    <textarea name="machinery_required" rows="3" class="{{ $inputClass }}">{{ old('machinery_required', $tomorrowPlan->machinery_required) }}</textarea>
                </div>

                <div>
                    <label class="{{ $labelClass }}">Drawing Required?</label>
                    <select name="drawing_required" class="{{ $inputClass }}" required>
                        <option value="0" {{ old('drawing_required', $tomorrowPlan->drawing_required) == 0 ? 'selected' : '' }}>No</option>
                        <option value="1" {{ old('drawing_required', $tomorrowPlan->drawing_required) == 1 ? 'selected' : '' }}>Yes</option>
                    </select>
                </div>

                <div>
                    <label class="{{ $labelClass }}">Client Approval Required?</label>
                    <select name="client_approval_required" class="{{ $inputClass }}" required>
                        <option value="0" {{ old('client_approval_required', $tomorrowPlan->client_approval_required) == 0 ? 'selected' : '' }}>No</option>
                        <option value="1" {{ old('client_approval_required', $tomorrowPlan->client_approval_required) == 1 ? 'selected' : '' }}>Yes</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text"
                           ...
                           value="{{ old('responsible_person', $tomorrowPlan->responsible_person) }}"
                           class="{{ $inputClass }}">
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-6">
            <div class="mb-5 rounded-xl bg-[#0F2A52] px-4 py-3 text-white">
                ...
                <p class="mt-1 text-xs text-blue-100">Update constraints and execution notes.</p>
            </div>

            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    <label class="{{ $labelClass }}">Risks / Constraints</label>
                    <textarea name="risks_constraints" rows="4" class="{{ $inputClass }}">{{ old('risks_constraints', $tomorrowPlan->risks_constraints) }}</textarea>
                </div>

                <div>
                    <label class="{{ $labelClass }}">Remarks</label>
                    <textarea name="remarks" rows="4" class="{{ $inputClass }}">{{ old('remarks', $tomorrowPlan->remarks) }}</textarea>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const savedLocation = {
        block: @json(old('project_block_id', $tomorrowPlan->project_block_id)),
        floor: @json(old('project_floor_id', $tomorrowPlan->project_floor_id)),
        unit: @json(old('project_unit_id', $tomorrowPlan->project_unit_id)),
        room: @json(old('project_room_id', $tomorrowPlan->project_room_id)),
        subspace: @json(old('project_subspace_id', $tomorrowPlan->project_subspace_id)),
    };
    ...
    const selectedActivityId = @json(old('activity_id', $tomorrowPlan->activity_id));
    ...
    function syncActivityUnit(force = false) {
        const selected = activities.find(item => String(item.id) === String(activity?.value || ''));

        if (selected?.unit && (force || !unitInput.value)) {
            unitInput.value = selected.unit;
        }
    }

    const selectedActivity = activities.find(item => String(item.id) === String(selectedActivityId));

    if (selectedActivity?.division_id) {
        division.value = String(selectedActivity.division_id);
    }

    rebuildActivities(selectedActivityId);
