...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-3xl font-bold text-gray-800">Edit Monthly Plan</h1>
        <p class="text-gray-500 mt-1">Update monthly planning target, resources and status.</p>
    </div>
...
<form method="POST" action="{{ route('monthly-plans.update', $monthlyPlan) }}">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <select name="project_id" class="border p-2 rounded w-full" required>
                ...
                @foreach($projects as $project)
                    <option value="{{ $project->id }}"
                        {{ old('project_id', $monthlyPlan->project_id) == $project->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <select name="plan_month" class="border p-2 rounded w-full" required>
                @for($m = 1; $m <= 12; $m++)
                    <option value="{{ $m }}"
                        {{ old('plan_month', $monthlyPlan->plan_month) == $m ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number"
                   name="plan_year"
                   value="{{ old('plan_year', $monthlyPlan->plan_year) }}"
        ...
        <div>
            ...
            <input type="date"
                   ...
                   value="{{ old('month_start_date', $monthlyPlan->month_start_date) }}"
                   class="border p-2 rounded w-full"
                   required>
        </div>
        ...
        <div>
            ...
            <input type="date"
                   name="month_end_date"
                   value="{{ old('month_end_date', $monthlyPlan->month_end_date) }}"
                   class="border p-2 rounded w-full"
                   required>
        </div>
            ...
            <select name="user_id" class="border p-2 rounded w-full">
                ...
                @foreach($engineers as $engineer)
                    <option value="{{ $engineer->id }}"
                        {{ old('user_id', $monthlyPlan->user_id) == $engineer->id ? 'selected' : '' }}>
                        {{ $engineer->name }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="activity_id"
                ...
                @foreach($activities as $activity)
                    <option value="{{ $activity->id }}"
                            ...
                            {{ old('activity_id', $monthlyPlan->activity_id) == $activity->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number"
                   ...
                   value="{{ old('planned_quantity', $monthlyPlan->planned_quantity) }}"
                   class="border p-2 rounded w-full"
                   required>
        </div>
        ...
        <div>
            ...
            <input type="text"
                   name="unit"
                   value="{{ old('unit', $monthlyPlan->unit) }}"
                   class="border p-2 rounded w-full">
        </div>
        ...
        <div>
            ...
            <input type="number"
                   name="planned_labour"
                   value="{{ old('planned_labour', $monthlyPlan->planned_labour) }}"
                   class="border p-2 rounded w-full">
        </div>
        ...
        <div>
            ...
            <select name="status" class="border p-2 rounded w-full" required>
                @foreach(['Planned', 'In Progress', 'Completed', 'Delayed'] as $status)
                    <option value="{{ $status }}"
                        {{ old('status', $monthlyPlan->status) == $status ? 'selected' : '' }}>
                        {{ $status }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div>
            ...
            <textarea name="materials_required"
                      rows="3"
                      class="border p-2 rounded w-full">{{ old('materials_required', $monthlyPlan->materials_required) }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="machinery_required"
                      rows="3"
                      class="border p-2 rounded w-full">{{ old('machinery_required', $monthlyPlan->machinery_required) }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="risks_constraints"
                      rows="4"
                      class="border p-2 rounded w-full">{{ old('risks_constraints', $monthlyPlan->risks_constraints) }}</textarea>
        </div>
        ...
        <div>
            ...
            <textarea name="remarks"
                      rows="4"
                      class="border p-2 rounded w-full">{{ old('remarks', $monthlyPlan->remarks) }}</textarea>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectedActivityId = "{{ old('activity_id', $monthlyPlan->activity_id) }}";
