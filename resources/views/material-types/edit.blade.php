...
<div class="mx-auto max-w-5xl">
    <div class="mb-5 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Update Product classification and operational master data.</p>
        </div>
    ...
    <form method="POST" action="{{ route('material-types.update', $materialType) }}">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <select id="material_product_group_id"
                        ...
                        @foreach($productGroups as $group)
                            <option value="{{ $group->id }}"
                                {{ (string) old('material_product_group_id', $materialType->material_product_group_id) === (string) $group->id ? 'selected' : '' }}>
                                {{ $group->group_name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="material_product_type_id"
                        ...
                        @foreach($productTypes as $type)
                            <option value="{{ $type->id }}"
                                ...
                                {{ (string) old('material_product_type_id', $materialType->material_product_type_id) === (string) $type->id ? 'selected' : '' }}>
                                {{ $type->type_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('material_type_code', $materialType->material_type_code) }}"
                           class="{{ $inputClass }}"
                ...
                <div>
                    ...
                    <input type="text"
                           name="inventory_type"
                           value="{{ old('inventory_type', $materialType->inventory_type) }}"
                ...
                <div>
                    ...
                    <select name="master_status" class="{{ $inputClass }}" required>
                        @foreach($masterStatuses as $status)
                            <option value="{{ $status }}"
                                {{ old('master_status', $materialType->master_status) === $status ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const selectedGroup = @json((string) old('material_product_group_id', $materialType->material_product_group_id));
    const selectedType = @json((string) old('material_product_type_id', $materialType->material_product_type_id));
